...
//Minified html code
const char webpage[] PROGMEM="<!DOCTYPE html><html><style>div.wm{position:relative;left:36px;height:1px}</style><head><title>Bumper Bot Controls</title><script>function drive(L,R,C=0){if(C==0){var req='?'+L+','+R+'!'}else{var req='?C,C!'+C+'&'}var xhr=new XMLHttpRequest();xhr.open('GET',req,!0);xhr.send()}"
	"function toggleTilt(){tilt=!tilt;if(tilt){document.getElementById('ttog').innerHTML='on'}else{document.getElementById('ttog').innerHTML='off'}}"
	...
	"event.preventDefault()},!0);window.addEventListener('deviceorientation',function(event){console.log('logit'+event.beta);if(tilt){drive(event.gamma-center_gamma+(event.beta),event.gamma-center_gamma-(event.beta));console.log('throt: '+(event.gamma-center_gamma)+event.beta)}"
